COPY (
    WITH resolved_trials AS (
        SELECT t.nct_id, t.phase, t.overall_status, t.start_date, t.completion_date, i.mechanism_class
        FROM trials t
        JOIN interventions i ON t.nct_id = i.nct_id
        WHERE t.overall_status NOT IN ('RECRUITING', 'ACTIVE_NOT_RECRUITING', 'NOT_YET_RECRUITING', 'ENROLLING_BY_INVITATION', 'UNKNOWN')
    ),
    all_phase_trials AS (
        SELECT DISTINCT t.nct_id, t.phase, i.mechanism_class
        FROM trials t
        JOIN interventions i ON t.nct_id = i.nct_id
        WHERE t.phase IN ('EARLY_PHASE1', 'PHASE1', 'PHASE2', 'PHASE3', 'PHASE4')
    ),
    phase_success AS (
        SELECT mechanism_class, COUNT(*) AS total_phased_trials,
    ...
    SELECT
        ps.mechanism_class, ps.total_phased_trials, ps.phase3_plus_trials,
        ROUND(100.0 * ps.phase3_plus_trials / ps.total_phased_trials, 1) AS phase3_plus_rate_pct,
        cs.total_resolved_trials, cs.completed_trials,
    ...
    WHERE ps.total_phased_trials >= 5
